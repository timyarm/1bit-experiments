"""GPTQ-initialized STE distillation for 1-bit Qwen3-1.7B (v2).

Two-phase training:
  Phase 1: Freeze scales, only train signs via identity STE (8 epochs)
  Phase 2: Unfreeze scales with tiny LR + clamping (4 epochs)

Key fix from v1: scales were exploding (ratio_med 78x vs target 2-3x) because
optimizer inflated scales instead of flipping signs. Freezing scales forces
the optimizer to flip signs, which is what Bonsai actually does.
"""
import modal
import json

app = modal.App("1bit-ste-distill-v2")
...
def run_ste_distillation():
    ...
    for name, module in list(student.named_modules()):
        ...
        bit.weight.data.copy_(module.weight.data.float())
    ...
    # Save initial teacher signs and scales for forensics
    init_signs = {}
    init_scales = {}
    init_scale_params = {}  # for scale clamping
    with torch.no_grad():
        for bname, bit in bit_modules.items():
            ...
            init_signs[bname] = s.cpu()
            init_scales[bname] = w_g.abs().mean(dim=2).cpu()
    ...
    for epoch in range(1, SIGN_EPOCHS + 1):
        ...
        if epoch % 4 == 0:
            ckpt = f"/data/qwen17b_ste_v2_epoch{epoch}.pt"
            torch.save({
    ...
    with open("/data/qwen17b_ste_distill_v2_results.json", "w") as f:
        json.dump(results, f, indent=2)
    vol.commit()

    print(f"\n{'=' * 60}")
    print("STE DISTILLATION v2 COMPLETE")
...
def main():
    print("GPTQ + STE Distillation v2 -- Qwen3-1.7B\n")
