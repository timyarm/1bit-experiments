...
# Hyperparameters
GROUP_SIZE = 128
N_STEPS = 5000         # Extended run with constant LR
BATCH_SIZE = 2         # Small — both models on GPU + gradients
LR_WEIGHT = 3e-3       # Sprint C winner: 3x higher LR + category weights
LR_SCALE = 5e-3
MAX_SEQ_LEN = 256      # Shorter for memory — activations for full model
N_CALIB = 256
KL_TEMP = 2.0          # Distillation temperature — softens teacher distribution
GRAD_ACCUM = 4         # Effective batch size = 2 × 4 = 8
EVAL_EVERY = 500       # Forensics + sample text every N steps
...
def run_e2e_qat():
    ...
    # Freeze quantized linear weights, unfreeze everything else
    ...
    # Unfreeze LayerNorm/RMSNorm only — tiny params that recalibrate activations
    # lm_head and embeddings stay FROZEN (too large, causes mode collapse on 256 samples)
    ln_params = []
    ...
    bit_trainable = sum(p.numel() for p in weight_params + scale_params)
    ln_trainable = sum(p.numel() for p in ln_params)
    print(f"  BitLinear params: {bit_trainable:,}", flush=True)
    print(f"  LayerNorm params: {ln_trainable:,}", flush=True)
    print(f"  Total trainable: {bit_trainable + ln_trainable:,}", flush=True)
    ...
    optimizer = torch.optim.AdamW([
        ...
        {"params": ln_params, "lr": 1e-4},  # LayerNorm adapts gently
    ], weight_decay=0.0)
    ...
    for step in range(N_STEPS):
        ...
        # Category-weighted gradients: FFN layers get 3x pressure (Sprint C winner)
        CAT_WEIGHTS = {
        ...
        if (step + 1) % EVAL_EVERY == 0 or step == N_STEPS - 1:
            ...
            if ppl < best_ppl:
                ...
                torch.save(ckpt, "/data/e2e_qat_best_checkpoint.pt")
                vol.commit()
    ...
    with open("/data/e2e_qat_results.json", "w") as f:
