...
# Hyperparameters
GROUP_SIZE = 128
N_STEPS = 5000         # Full run — extending sprint C winner
BATCH_SIZE = 2
LR_WEIGHT = 3e-3       # 3x higher than baseline
LR_SCALE = 5e-3
MAX_SEQ_LEN = 256
N_CALIB = 256
KL_TEMP = 2.0
GRAD_ACCUM = 4
EVAL_EVERY = 500       # 10 evals over 5000 steps
...
def run_e2e_qat():
    ...
    for step in range(N_STEPS):
        ...
        if (step + 1) % EVAL_EVERY == 0 or step == N_STEPS - 1:
            ...
            if ppl < best_ppl:
                ...
                torch.save(ckpt, "/data/sprint_c_best_checkpoint.pt")
                vol.commit()
    ...
    with open("/data/sprint_c_both_results.json", "w") as f:
